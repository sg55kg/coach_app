...
export class User {
    static build(userDTO: UserDTO) {
        ...
        user.athleteData = userDTO.athleteData
            ? AthleteData.createFrom(userDTO.athleteData)
            : null;
        user.coachData = userDTO.coachData
            ? CoachData.createFrom(userDTO.coachData)
            : null;
        ...
        user.members = userDTO.members
            ? userDTO.members.map(m => ChatMember.createFrom(m))
            : [];

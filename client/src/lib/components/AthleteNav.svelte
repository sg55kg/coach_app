...
<div
    class="overlay fixed inset-0 z-40 transform overflow-hidden ease-in-out {showNav
        ? 'translate-x-0 opacity-100 transition-opacity duration-300'
        : 'translate-x-full opacity-0 transition-all delay-100'}"
>
    <div
        class="absolute left-0 h-full w-9/12 transform bg-gray-100 shadow-xl transition-all delay-200 duration-200 ease-in-out lg:w-3/12 {showNav
            ? 'translate-x-0'
            : 'translate-x-[-30em]'}"
    >

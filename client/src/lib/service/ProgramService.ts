...
export class ProgramService {
    ...
    static updateExercise = async (exercise: Exercise) => {
        const { data } = await srPut<ExerciseDTO>(`/api/exercise/${exercise.id}`, exercise)
    ...
    static addExerciseComment = async (comment: ExerciseComment) => {
        const { data } = await srPost<ExerciseComment>(`/api/exercise/${comment.exerciseId}/comment`, comment)
        return data
    }
    ...
    static searchProgramsByName = async (coachId: string, name: string) => {
        const { data } = await srGet<ProgramDTO[]>(`/api/program?coach=${coachId}&name=${name}`)
        return data.map(d => DisplayProgram.build(d))
    }

    static getProgramStats = async (programId: string) => {
        const { data } = await srGet<AthleteProgramStatsDTO>(`/api/stats/${programId}`)
        return AthleteProgramStats.createFrom(data);
    }

    static getWeeklyProgramStats = async (programId: string) => {
        const { data } = await srGet<AthleteProgramStatsDTO[]>(`/api/stats/${programId}/weekly`)

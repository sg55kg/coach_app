...
import type {UserDTO} from "../classes/user";
...
export default class UserService {
    ...
    static updateAthleteRecords = async (record: AthleteRecord, id: string) => {
        const { data } = await srPut<AthleteRecordDTO[]>(`/api/athlete/${id}/record`, record)
    ...
    static updateAthleteData = async (athlete: AthleteData) => {
        ...
        const { data } = await srPut<AthleteDataDTO>(`/api/athlete/${id}`, body)
    ...
    static fetchAthleteStats = async (athleteId: string) => {
        const { data } = await srGet<AthleteProgramStatsDTO[]>(`/api/athlete/${athleteId}/stats`)
    ...
    static createAthleteRecord = async (athleteId: string, newRecord: AthleteRecord) => {
        const { data } = await srPost<AthleteRecordDTO>(`/api/athlete/${athleteId}/record`, newRecord)
        return AthleteRecord.createFrom(data)
    }

    static createAthleteRecords = async (athleteId: string, records: AthleteRecord[]) => {
        const { data } = await srPost<AthleteRecordDTO[]>(`/api/athlete/${athleteId}/record/list`, records)
